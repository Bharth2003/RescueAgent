...
The map is `folium` + `streamlit-folium` on OpenStreetMap tiles: a dashed grey line for
the driver's approach, a solid line for the delivery leg, square markers for kitchen and
shelter, and a pulsing marker on arrival. Before anything is dispatched, all 40 drivers
are plotted at their current positions; once a driver accepts, the rest dim and the
assigned driver is tracked.
...
three-minute pitch: 30 s to pickup, 40 s to the shelter, 8 s auto-accept, 9 s
auto-handover — about 1 min 30 s end to end. Constants at the top of `app.py`.
...
aws configure                    # credentials for Amazon Bedrock
```

Enable the model in the Bedrock console: region `eu-west-2`, model
`qwen.qwen3-235b-a22b-2507-v1:0`.

Optional but recommended before a live demo:
...
The sidebar has a **Call Bedrock agent** toggle. Off, the deterministic tool pipeline
runs alone — same trace, same map, no network call. Useful when venue wifi is unreliable.

## Demo path (about 1 min 30 s)
...
- Driver movement is simulated by interpolating the OSRM polyline. There is no GPS feed;
  wiring one in would replace `point_at` and nothing else.
- Streamlit redraws the map at roughly 1 fps.
